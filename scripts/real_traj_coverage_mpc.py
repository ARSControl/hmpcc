...
def gmm_human_cost_func(x, means, covs, weights, alpha=0.1):
  prob = 0.0
  for i in range(means.shape[0]):
    cov_i = ca.reshape(covs[i, :], 2, 2)
    coeff = 1 / ca.sqrt((2 * ca.pi) ** 2 * ca.det(cov_i))
    diff = ca.reshape(x, 1, 2) - means[i, :]
    dist = diff @ ca.inv(cov_i) @ diff.T
    prob += weights[i] * coeff * ca.exp(-0.5 * dist)
...
# load human GMMs
gmms_path = "output/config/TP/FlowChain/CIF_separate_cond_v_trajectron/eth/visualize/density_map/gmms/"
gmm_0 = np.load(gmms_path + "sq_pdf_0_7.npy")
print("gmm shape: ", gmm_0.shape)
T = gmm_0.shape[0]
start_idx = 819
end_idx = 913
NUM_STEPS = end_idx - start_idx + 1
human_means = np.zeros((NUM_STEPS, T, 2))
human_covariances = np.zeros((NUM_STEPS, T, 2, 2))
ids_list = np.arange(start_idx, end_idx)
for i in range(NUM_STEPS-1):
  gmm = np.load(gmms_path + f"ped_pdf_0_{ids_list[i]}.npy")
  human_means[i, :, :] = gmm[:, :2]
  human_covariances[i, :, :, :] = gmm[:, 2:6].reshape(T, 2, 2)
...
traj_path = "src/data/TP/raw_data/mio/pedsim0.txt"
human_t = np.loadtxt(traj_path, usecols=(2,3))
# only keep values with predictions
human_traj = human_t[start_idx:end_idx+1, :]
print("human t shape: ", human_traj.shape)
assert human_traj.shape[0] == NUM_STEPS, "Human trajectory length does not match the number of steps"
...
for s in range(NUM_STEPS):
  ...
  for idx in range(ROBOTS_NUM):
    ...
    # human_cost_expr = gmm_human_cost_func(x0, m_var, c_var, w_var, alpha=alpha_human)
    # human_cost_fn = ca.Function('human_cost', [x0, m_var, c_var, w_var], [human_cost_expr])

    # 4. Build optimizaiton problem
    obj = 0.0
    x_curr = x0
    g_list = []
    human_mean_s = human_means[s]
    human_cov_s = human_covariances[s]
    # print("human_mean_s shape: ", human_mean_s.shape)
    # print("Human cov s shape: ", human_cov_s.shape)
    for k in range(T):
      # print("Planning step: ", k)
      obj += cost_fn(x_curr) #+ human_cost_fn(x_curr, human_mean_s[k], human_cov_s[k], human_weights_s[k])
      ...
      diff = x_curr - human_mean_s[k]
      
      mahalanobis_dist_sq = diff.T @ ca.inv(human_cov_s[k]) @ diff
      prob = 0.05  # Desired probability of collision
      coeff = ca.sqrt(ca.det(2*ca.pi * human_cov_s[k]))
      vol = 4/3 * ca.pi * (4*Ds)**3
  ...
  # Z_human = gmm_pdf(X, Y, human_mean_s[-1], human_cov_s[-1].reshape(components_num, 2, 2), human_weights_s[-1])

  ax.contourf(X, Y, Z.reshape(X.shape), levels=10, cmap='YlOrRd', alpha=0.75)
  # ax.contourf(X, Y, Z_human.reshape(X.shape), levels=10, cmap='Blues', alpha=0.75)
  ...
  for t in range(T):
    ...
    draw_ellipse(human_means[s, t, :], human_covariances[s, t], n_std=1, ax=ax, alpha=alpha, edgecolor="tab:purple")
