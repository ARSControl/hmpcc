...
for s in range(NUM_STEPS):
  ...
  for idx in range(ROBOTS_NUM):
    ...
    for k in range(T):
      planned_traj[k+1, :] = dynamics(planned_traj[k, :], u_opt[k, :], dt).full().flatten()
    planned_trajectories[:, idx, :] = planned_traj
    points[idx, :] = dynamics(positions_now[idx, :], u_opt[0, :], dt).full().flatten()
